...
my $N=6; my $VARNO=57; my $AXNO=240;
...
sub read_args {
    ...
    $rayfile=$ARGV[0]; $filestub=$ARGV[1];
...
sub parse_axname {
    ...
    if($txt =~ /^f\(\{([\d,\s]+)\}\) \+ f\(\{([\d,\s]+)\}\)/ ){
        my($v1,$v2)=($1,$2);
        $v1=bitset($v1); $v2=bitset($v2); # A|C=v1, B|C=v2
        my $C=$v1&$v2; my $A=$v1 & ~$C; my $B=$v2 & ~$C;
...
sub get_axorder { # simple basic order
    ...
    my @id=(); for my $i(0 .. $AXNO-1){$id[$i]=$i;}
...
sub get_altorder {
    ...
    for my $i(0 .. $N-2){ for my $j($i+1 .. $N-1){
       my @rest=();
       for my $k(0 .. $N-1){
           next if($k==$i || $k==$j);
           push @rest,$k;
...
sub find_base {
     ...
     my $filled=0; my $nextlistno=0; my @M; my $scan=0;
     while($filled<$VARNO){
         if($nextlistno==$AXNO){
             if($scan<2){$scan++; $nextlistno=0; next; }
             die "Cannot generate base ($filled, $VARNO)\n";
         }
         my $nextax=$ORDER[$nextlistno]; $nextlistno++;
...
sub gcd { # x,y non-negative integers, fast Euclidean algorithm
    ...
    while(1){
       ...
       $x %= $y; if($x==0){return $y;} if($x==1){ return 1; }
    }    
}
...
sub integrify {
    ...
    for my $i(0 .. $VARNO-1){
        next if($ray->[$i]>=0);
        $signs[$i]=1; $ray->[$i]=-$ray->[$i];
    }
    my $d=1;
    for my $i(0 .. $VARNO-1){
        my $v=$d*$ray->[$i]; my $iv=int((1+5e-10)*$v+2e-9*$d);
        $iray[$i]=$iv; next if(is_zero(($v-$iv)/$d));
    ...
    if($d!=1){
       for my $i(0 .. $VARNO-1){
          my $v=$d*$ray->[$i]; my $iv=int($v+0.01);
          $iray[$i]=$iv;
          is_zero(($v-$iv)/$d) || die "integrify problem: v=$v, iv=$iv\n";
       }
    }
    $d=gcd($iray[0],$iray[1]);my $idx=2;
    while($d!=1 && $idx<$VARNO){ $d=gcd($iray[$idx],$d); $idx++; }
    if($d>1){for my $i(0 .. $VARNO-1){$iray[$i] /= $d; } }
    for my $i(0 .. $VARNO-1){
...
sub find_initial_rays {
    my @M=(); my $idx=0; my @skipvar=(0) x $VARNO;
    for my $i(0 .. $AXNO-1){
    ...
    for my $i(0 .. $VARNO-1){
        ...
        for my $row(0 .. $VARNO-1){
           my $idx=-1;
           for my $s(0 .. $VARNO-1){if(not_zero($M[$row]->[$s])){$idx=$s;last;}}
...
sub write_initial_rays {
    ...
    open(FILE,">",$file) || die "Cannot create file $file\n";
      ...
      for my $i(0 .. $AXNO-1){if($INBASE[$i]==1){$extracol++; }}
      $extracol==1 || die "no extra axiom is found ($extracol)\n";
      print FILE "$extracol";
      for my $i(0 .. $AXNO-1){if($INBASE[$i]==1){ print FILE " $i";}}
...
sub generate_neighbor_file {
    ...
    my $total=$itno; for my $i(0 .. $AXNO-1){ $total++ if($INBASE[$i]==0); }
    for my $i(0 .. $AXNO-1){
        next if($INBASE[$ORDER[$i]]!=0);
...
my @varnamearr=(); my @vidxarr=(); my @complvarnamearr=();
...
sub addvperm_ifnew {
    ...
    foreach my $vp(@VPERMS){
        my $same=1;
        for my $i(0 .. -1+$VARNO){ if($p->[$i]!=$vp->[$i]){ $same=0; last; }}
        if($same){ return 0; }
    }
    push @VPERMS,$p;
    return 1;
...
sub generate_varperms {
    my @id=(); for my $i(0 .. -1+$VARNO){$id[$i]=$i;}
    push @VPERMS,\@id;
...
sub dual {
    my($ray)=@_;
    my $full=""; for my $i(0 .. $N-1){ $full .= "$i"; }
    my @dual=(); my $K=$ray->[vidx($full)];
    for my $j(0 .. -1+$VARNO){
        my $cmp=vcompl($j); $dual[$j]=-$K;
        if($cmp eq ""){ 
...
sub symmno { # how many symmetries the ray has
    my($ray)=@_;
    my $symm=0; my $base=join(',',@$ray);
    my @new=(); my $dual=dual($ray);
    for my $p(@VPERMS){
...
sub handle_ray {
    ...
    my $rno=0; if($stored =~ /(\d+) rays appended/){$rno=$1;}
...
sub process_rayfile {
    my @RAYS=(); my @RAYLABELS=(); my $total=0;
    read_logfile();
