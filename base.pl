...
sub read_args {
    ...
    scalar @ARGV==1 || usage("missing filestub");
    $filestub=$ARGV[0];
...
sub get_lexminorder {
    ...
    for my $iidx(0 .. $VARNO-1){
        my $ii=$VARNO-1-$iidx;
        my $s=int(rand($ii+1));
        my $t=$VARPERM[$ii]; $VARPERM[$ii]=$VARPERM[$s]; $VARPERM[$s]=$t;
    }
    for my $i(0 .. $AXNO-1){
       $ORDER[$i]=$i;
    }
    ...
    while($doit){
        ...
        for my $i(0 .. $AXNO-2){
           if(lexcmp($ORDER[$i],$ORDER[$i+1])<0){
              $doit=1;
              my $t=$ORDER[$i]; $ORDER[$i]=$ORDER[$i+1]; $ORDER[$i+1]=$t;
           }
        }
    }
}
...
sub find_base {
     ...
     my $filled=0; my $nextlistno=0; my @M;
     while($filled<$VARNO){
         if($nextlistno==$AXNO){
...
sub write_initial_rays {
    ...
    open(FILE,">",$file) || die "Cannot create file $file\n";
      ...
      for my $i(0 .. $AXNO-1){if($INBASE[$i]==1){ print FILE " $i"; $hcols++}}
      print FILE "\n";
