...
{const char *w, *input,*output;
    verbose=0; checkrank=0; noorbit=0;
...
{int v=0; const int *ax=axioms[axno];
...
/* =============================================================== */
/* statistics */
static int negidx; static size_t combchkyes;

static void statistics(void)
{static float lastreport=0.0; float elapsed;
...
{int pairs_checked=0;
    ...
    for(negidx=0;negidx<negrays;negidx++){
       ...
       if(pairs_checked>=5000000){
           pairs_checked=0; if(verbose) statistics();
       }
...
{int rank=0;
    ...
    // go over all rows
    for(int j=0;j<axn_given;j++){double *Mj,maxM; int pivot;
       // find the maximal absolut value in row j
       Mj=&(M[j][0]); maxM=Mj[0]; if(maxM<0.0){maxM=-maxM;} pivot=0;
       for(int i=1;i<DIM;i++){ double w=Mj[i];
           if(w<0.0){
...
int main(int argc,const char*argv[])
{
    ...
    find_adjacent_rays();
    fclose(ofile);
    printf("Done, rays=%d, idx=%d, time=",newrays,negidx); printtime(elapsed_time(0)); printf("\n");
    return 0;
}
